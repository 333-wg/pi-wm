...
function decryptedCatalog(encoded: string, secret: string): unknown {
	...
	return JSON.parse(Buffer.concat([decipher.update(Buffer.from(stored.data, "base64url")), decipher.final()]).toString("utf8"));
}
...
describe("CustomModelRegistry", () => {
	...
	it("keeps keys out of metadata and provider catalog summaries", async () => {
		...
		expect(registry.list()).toEqual([{ model: { provider: config.provider, id: config.id }, name: config.name, reasoning: false, input: ["text"], contextWindow: 32_000, maxOutputTokens: 4096, authenticated: true, custom: true }]);
		...
		expect(registry.registrations()[0]?.config).toMatchObject({ apiKey: config.apiKey, baseUrl: config.baseUrl });
	});
	...
	it("persists a reusable service before any model is selected", async () => {
		...
		try {
			const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async () => new Response(JSON.stringify({ data: [{ id: "model-a", name: "Model A" }] }), { status: 200 }));
			const filePath = join(root, "models.enc");
			const first = new CustomModelRegistry({ filePath, encryptionKey: "master-key" });
			const discovery = await first.discover({ baseUrl: "https://gateway.example/v1", apiKey: "service-key" });
			expect(first.services()).toEqual([{ provider: discovery.provider, baseUrl: discovery.baseUrl, api: discovery.api, authenticated: true, modelCount: 0 }]);
			const raw = await readFile(filePath, "utf8");
			...
			await expect(second.removeService(discovery.provider)).rejects.toMatchObject({ protocolCode: "conflict" });
	...
	it("migrates legacy model arrays into reusable services", async () => {
		...
		try {
			...
			await registry.load();
			expect(registry.services()).toEqual([{ provider: config.provider, baseUrl: config.baseUrl, api: config.api, authenticated: true, modelCount: 1 }]);
			expect(decryptedCatalog(await readFile(filePath, "utf8"), "master-key")).toMatchObject({ version: 2, services: [{ provider: config.provider, apiKey: config.apiKey }], models: [{ provider: config.provider, id: config.id }] });
		} finally {
			await rm(root, { recursive: true, force: true });
		}
	});
	...
	it("rejects credential-bearing or unsupported endpoints", async () => {
		...
		await expect(registry.set({ ...config, baseUrl: "https://user:pass@example.test/v1" })).rejects.toThrow("credentials");
	});

	it("does not merge models from the same provider when connection settings differ", async () => {
		const registry = new CustomModelRegistry();
		await registry.set(config);
		await expect(registry.set({ ...config, id: "other-model", apiKey: "different-key" })).rejects.toThrow("share base URL");
	});
	...
	it("discovers OpenAI-compatible gateway models with bearer authentication", async () => {
		...
		const registry = new CustomModelRegistry();
		const result = await registry.discover({ baseUrl: "https://gateway.example/v1", apiKey: "gateway-key" });
		expect(result.api).toBe("openai-completions");
		expect(result.baseUrl).toBe("https://gateway.example/v1");
		expect(result.provider).toMatch(/^custom-gateway-example-/);
		expect(result.models).toEqual([{ id: "model-b", name: "Model B" }, { id: "model-a", name: "model-a" }]);
	...
	it("selects the Pi Responses adapter for the official OpenAI endpoint", async () => {
		vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({ data: [{ id: "gpt-test" }] }), { status: 200 }));
		const registry = new CustomModelRegistry();
		const result = await registry.discover({ baseUrl: "https://api.openai.com/v1", apiKey: "official-key" });
	...
	it("normalizes a root gateway URL when model discovery succeeds under v1", async () => {
		...
		const registry = new CustomModelRegistry();
		const result = await registry.discover({ baseUrl: "https://root-gateway.example", apiKey: "gateway-key" });
	...
	it("uses the versioned Anthropic model and message endpoints", async () => {
		const fetchMock = vi.spyOn(globalThis, "fetch")
			.mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ id: "claude-test", display_name: "Claude Test" }] }), { status: 200 }))
			.mockResolvedValueOnce(new Response(JSON.stringify({ content: [] }), { status: 200 }));
		const registry = new CustomModelRegistry();
		const discovery = await registry.discover({ baseUrl: "https://api.anthropic.com", apiKey: "anthropic-key" });
		expect(discovery.api).toBe("anthropic-messages");
		expect(fetchMock.mock.calls[0]?.[0]).toBe("https://api.anthropic.com/v1/models");
		await registry.set({ ...config, provider: discovery.provider, id: "claude-test", name: "Claude Test", api: discovery.api, baseUrl: discovery.baseUrl, apiKey: "anthropic-key" });
