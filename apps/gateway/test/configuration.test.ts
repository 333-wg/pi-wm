...
describe("gateway configuration", () => {
	it("places implicit Gateway state in the operating-system user data directory", () => {
		expect(
			defaultDataDirectory({
				platform: "win32",
		...
		).toBe(resolve("C:\\Users\\Alice", "AppData", "Local", "Wuming"));
		expect(
			defaultDataDirectory({
				platform: "win32",
				homeDirectory: "C:\\Users\\Alice",
				environment: { LOCALAPPDATA: "D:\\LocalAppData" },
			})
		).toBe(resolve("D:\\LocalAppData", "Wuming"));
		expect(
			defaultDataDirectory({
				platform: "darwin",
		...
		).toBe(resolve("/Users/alice", "Library", "Application Support", "Wuming"));
		expect(
			defaultDataDirectory({
				platform: "linux",
		...
		).toBe(resolve("/home/alice", ".local", "share", "Wuming"));
		expect(
			defaultDataDirectory({
				platform: "linux",
				homeDirectory: "/home/alice",
				environment: { XDG_DATA_HOME: "/mnt/data" },
			})
		).toBe(resolve("/mnt/data", "Wuming"));
	});
	...
	it("rejects duplicate workspace and model identities", () => {
		expect(() =>
			configuredWorkspaces("D:\\fallback", {
				WUMING_WORKSPACES_JSON: JSON.stringify([
		...
		expect(() =>
			configuredModels(fallbackModel, {
				WUMING_MODELS_JSON: JSON.stringify([
	...
	it("parses and validates web search providers", () => {
		...
		expect(configuredWebSearch({ WUMING_WEB_SEARCH_PROVIDER: "bing" })).toEqual({
			provider: "bing",
		});
		expect(configuredWebSearch({ WUMING_WEB_SEARCH_PROVIDER: "duckduckgo" })).toEqual({
			provider: "duckduckgo",
		});
		expect(
			configuredWebSearch({
				WUMING_WEB_SEARCH_PROVIDER: "brave",
				WUMING_WEB_SEARCH_API_KEY: "secret",
			})
		).toEqual({
			provider: "brave",
			apiKey: "secret",
		...
		expect(
			configuredWebSearch({
				WUMING_WEB_SEARCH_PROVIDER: "searxng",
		...
		expect(() => configuredWebSearch({ WUMING_WEB_SEARCH_PROVIDER: "unknown" })).toThrow(
			/must be bing, duckduckgo, brave, or searxng/
		);
	});
});
