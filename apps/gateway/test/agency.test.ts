...
function invoke(entry: ToolDefinition, toolCallId: string, params: Parameters<ToolExecute>[1], signal?: AbortSignal): ReturnType<ToolExecute> {
...
function fakeRunner(options: { summary?: SubagentSummary; drain?: () => Promise<number>; depth?: number } = {}): FakeRunner {
	const runner: FakeRunner = {
		...
		async createSubagent(input) {
			...
			return { subagent: unfinished({ status: "running", name: input.name ?? "Subagent", task: input.task }) };
		},
...
describe("update_plan", () => {
	it("renders progress, marks the active step, and echoes the explanation", async () => {
		...
		const result = await invoke(plan, "call-1", {
	...
	it("rejects a plan with two steps in progress", async () => {
		...
		await expect(invoke(plan, "call-4", {
...
describe("subagent", () => {
	...
	it("delegates inline, drains the child, and returns its report with usage", async () => {
		...
		const result = await invoke(subagent, "call-1", { task: "Find every caller", name: "callers", cost_budget_usd: 0.5 }, signal());
		...
		expect(text(result)).toBe("Subagent callers completed using 12 token(s), $0.0125.\n\nThree callers, all in packages/orchestrator.");
	...
	it("propagates a drain failure that is not a lease conflict", async () => {
		...
		await expect(invoke(subagent, "call-1", { task: "Find every caller" }, signal())).rejects.toThrow("store is closed");
	});

	it("surfaces failure and cancellation as tool errors, and an unfinished child as a note", async () => {
		const rootSnapshot = snapshot();
		const failed = tool(createAgencyTools({ snapshot: rootSnapshot, runner: fakeRunner({ summary: unfinished({ status: "failed", error: "budget exhausted" }) }) }), "subagent");
		await expect(invoke(failed, "call-1", { task: "t" }, signal())).rejects.toThrow("Subagent callers failed after 12 token(s), $0.0125: budget exhausted");

		const cancelled = tool(createAgencyTools({ snapshot: rootSnapshot, runner: fakeRunner({ summary: unfinished({ status: "cancelled" }) }) }), "subagent");
		await expect(invoke(cancelled, "call-2", { task: "t" }, signal())).rejects.toThrow("Subagent callers was cancelled after 12 token(s), $0.0125.");

		const waiting = tool(createAgencyTools({ snapshot: rootSnapshot, runner: fakeRunner({ summary: unfinished({ status: "awaiting_approval" }) }) }), "subagent");
