...
export function createAgencyTools(options: AgencyToolOptions): ToolDefinition[] {
		...
		defineTool({
			...
			parameters: Type.Object({
				plan: Type.Array(
					...
					{ minItems: 1, maxItems: 40, description: "The complete plan, in order. Replaces any previous plan." },
				),
				explanation: Type.Optional(Type.String({ maxLength: 2000, description: "Why the plan changed, when it is not obvious" })),
			}),
	...
	tools.push(defineTool({
		...
		parameters: Type.Object({
			task: Type.String({ minLength: 1, maxLength: 20_000, description: "The complete instruction, including what to report back" }),
			name: Type.Optional(Type.String({ minLength: 1, maxLength: 200, description: "Short label shown in the UI" })),
			cost_budget_usd: Type.Optional(Type.Number({ exclusiveMinimum: 0, maximum: 1000, description: "Cap on this subagent's spend; defaults to the session's remaining budget" })),
			token_budget: Type.Optional(Type.Integer({ minimum: 1000, maximum: 100_000_000, description: "Cap on this subagent's tokens; defaults to the session's remaining budget" })),
		}),
		async execute(toolCallId, params, signal) {
			...
			if (summary.status === "failed") throw new Error(`Subagent ${summary.name} failed after ${usage}: ${summary.error ?? "no error reported"}`);
			if (summary.status === "cancelled") throw new Error(`Subagent ${summary.name} was cancelled after ${usage}.`);
			if (summary.status !== "completed") {
				return textResult(`Subagent ${summary.name} is still ${summary.status} (id ${subagentId}). Its report is not available yet; continue without it or ask the user to check the Agents tab.`, { subagentId, status: summary.status });
			}
