...
import type { GatewayAuth, GatewayPrincipal } from "./auth.js";
import { tokenFromProtocols } from "./auth.js";
...
export interface GatewayCustomModelService {
	...
	discover(connection: CustomModelConnection): Promise<{ provider: string; baseUrl: string; api: CustomModelApi; models: CustomModelCandidate[]; latencyMs: number }>;
	refreshService(provider: string): Promise<{ provider: string; baseUrl: string; api: CustomModelApi; models: CustomModelCandidate[]; latencyMs: number }>;
...
function protocolError(error: unknown): ProtocolError {
	...
	if (error instanceof OrchestratorError) {
		const code =
			error.code === "not_found"
				? "not_found"
				: error.code === "idempotency_conflict" || error.code === "conflict" || error.code.includes("lease")
					? "conflict"
	...
	if (error && typeof error === "object" && "code" in error && (error as { code?: string }).code === "approval_denied") {
		return { code: "conflict", message: error instanceof Error ? error.message : "Approval denied", retryable: false };
	}
	if (error && typeof error === "object" && "code" in error) {
		const artifactCode = (error as { code?: string }).code;
		if (["not_found", "forbidden", "invalid", "too_large"].includes(artifactCode ?? "")) {
			return {
				code: artifactCode === "not_found" ? "not_found" : artifactCode === "forbidden" ? "forbidden" : "invalid_request",
...
export class GatewayServer implements AsyncDisposable {
	...
	constructor(options: GatewayServerOptions) {
		...
		this.#unsubscribeStore = this.#store.subscribeEvents((stored) => this.#broadcastStoredEvent(stored));
	...
	async #httpRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
		try {
			...
			if (upload && request.method === "POST") {
				if (!this.#artifacts) throw Object.assign(new Error("Artifact service is unavailable"), { httpStatus: 501 });
				...
				if (typeof encodedName !== "string") throw Object.assign(new Error("X-Wuming-File-Name is required"), { httpStatus: 400 });
				let name: string;
				try {
				...
				const created = await this.#artifacts.create({
					...
					name,
					...(request.headers["content-type"] ? { suppliedMimeType: request.headers["content-type"] } : {}),
					content,
				});
			...
			if (download && request.method === "GET") {
				...
				if (!metadata) throw Object.assign(new Error(`Artifact ${artifactId} does not exist`), { httpStatus: 404 });
			...
			if (workspaceMatch && request.method === "GET") {
				...
				if (file) {
					this.#json(response, 200, await this.#workspace.readFile(workspaceId, this.#queryPath(url)));
					return;
				}
				if (gitStatus) {
				...
				if (path && path.length > 4000) throw Object.assign(new Error("Workspace path is too long"), { httpStatus: 400 });
			...
			if (!response.headersSent) this.#json(response, status, { error: error instanceof Error ? error.message : "Request failed" });
	...
	#queryPath(url: URL, fallback?: string): string {
		...
		if (path === undefined || path.length === 0) throw Object.assign(new Error("Workspace path is required"), { httpStatus: 400 });
		if (path.length > 4000) throw Object.assign(new Error("Workspace path is too long"), { httpStatus: 400 });
		return path;
	}
	...
	async #readRequestBody(request: IncomingMessage): Promise<Buffer> {
		...
		for await (const raw of request) {
			...
			size += chunk.length;
			if (size > this.#maxArtifactBytes) throw Object.assign(new Error("Artifact exceeds the upload limit"), { httpStatus: 413 });
			chunks.push(chunk);
		}
	...
	#httpErrorStatus(error: unknown): number {
		if (error && typeof error === "object" && "httpStatus" in error) return Number((error as { httpStatus: number }).httpStatus);
		if (error && typeof error === "object" && "code" in error) {
	...
	#accept(ws: WebSocket, principal: GatewayPrincipal): void {
		...
		this.#logger.log("info", "gateway.connection.opened", { connectionId: connection.connectionId, principalId: principal.id });
		ws.on("message", (data, isBinary) => {
			if (isBinary) return ws.close(1003, "JSON text messages required");
			void this.#message(connection, data);
		});
		ws.on("close", () => {
			this.#logger.log("info", "gateway.connection.closed", { connectionId: connection.connectionId, principalId: connection.principal.id });
	...
	async #message(connection: ConnectionState, raw: RawData): Promise<void> {
		...
		if (!checkClientMessage.Check(value)) return connection.ws.close(1002, "Invalid protocol message");
		...
		if (!connection.hello) {
			...
			if (message.protocolVersion !== PROTOCOL_VERSION) {
				this.#send(connection, {
					type: "hello_error",
					error: { code: "invalid_request", message: "Unsupported protocol version", retryable: false },
				});
				return connection.ws.close(1002, "Unsupported protocol version");
			}
			connection.hello = true;
			this.#logger.log("debug", "gateway.hello.accepted", { connectionId: connection.connectionId, clientId: message.clientId, resume: Boolean(message.resumeCursor) });
			this.#send(connection, {
				type: "hello",
	...
	async #terminalMessage(connection: ConnectionState, message: TerminalClientMessage): Promise<void> {
		...
		try {
			if (message.type === "terminal.create") {
				...
				connection.terminalUnsubscribers.add(terminal.listen(message.terminalId, connection.connectionId, (value) => this.#send(connection, value)));
				this.#send(connection, ready);
				return;
			}
			const owner = this.#terminalOwner(connection, message.terminalId);
			if (message.type === "terminal.attach") {
				const ready = terminal.attach({ ...message, owner, connectionId: connection.connectionId, send: (value) => this.#send(connection, value) });
				connection.terminalUnsubscribers.add(terminal.listen(message.terminalId, connection.connectionId, (value) => this.#send(connection, value)));
				this.#send(connection, ready);
				return;
			}
			...
			this.#send(connection, {
				type: "terminal.error",
				...(message.type === "terminal.create" || message.type === "terminal.attach" || message.type === "terminal.close" ? { requestId: message.requestId } : {}),
				...(message.type === "terminal.input" || message.type === "terminal.resize" || message.type === "terminal.close" ? { terminalId: message.terminalId } : {}),
	...
	#terminalOwner(connection: ConnectionState, terminalId: string): { principalId: string; workspaceId: string } {
		const workspaceId = this.#terminal?.workspaceFor(terminalId, connection.principal.id);
		if (!workspaceId) throw Object.assign(new Error(`Terminal ${terminalId} is unavailable`), { protocolCode: "forbidden" });
	...
	async #request(connection: ConnectionState, requestId: string, idempotencyKey: string, command: Command): Promise<void> {
		const traceId = this.#idFactory();
		const startedAt = this.#clock();
		const sessionId = "sessionId" in command && typeof command.sessionId === "string" ? command.sessionId : undefined;
		this.#logger.log("info", "gateway.request.received", { traceId, requestId, idempotencyKey, connectionId: connection.connectionId, principalId: connection.principal.id, command: command.type, ...(sessionId === undefined ? {} : { sessionId }) });
		try {
			let result = await this.#dispatch(connection, idempotencyKey, command);
			const operationTracked = command.type === "turn.prompt" || command.type === "turn.steer" || command.type === "turn.follow_up" || command.type === "subagent.create";
			const operationId = operationTracked && sessionId !== undefined ? this.#store.listOperations(sessionId, 1)[0]?.id : undefined;
			if (command.type === "subagent.create" && result.type === "subagent.created") {
				...
				if (command.wait) result = { type: "subagent.created", subagent: await run() };
					...
					if (!(error instanceof OrchestratorError && error.code === "lease_conflict")) {
						this.#logger.log("error", "gateway.subagent.run_failed", { sessionId: subagent.sessionId, parentSessionId: command.sessionId, error });
						this.#onError(error);
					}
				});
			}
			...
			if (command.type === "turn.prompt" || command.type === "turn.steer" || command.type === "turn.follow_up") {
				void this.#orchestrator.drainSession(command.sessionId, undefined, traceId).catch((error) => {
					if (!(error instanceof OrchestratorError && error.code === "lease_conflict")) {
						this.#logger.log("error", "gateway.turn.drain_failed", { sessionId: command.sessionId, error });
						this.#onError(error);
					}
				});
			}
		} catch (error) {
			this.#logger.log("error", "gateway.request.failed", { traceId, requestId, command: command.type, durationMs: Math.max(0, this.#clock() - startedAt), ...(sessionId === undefined ? {} : { sessionId }), error });
			this.#send(connection, { type: "response", requestId, ok: false, error: protocolError(error) });
		}
	}

	async #dispatch(connection: ConnectionState, idempotencyKey: string, command: Command): Promise<CommandResult> {
		switch (command.type) {
			case "workspace.list":
			...
			case "tool.list": {
				...
				return { type: "tool.list", workspaceId: command.workspaceId, runtime: this.#tools.runtime, tools };
			}
			case "skill.list": {
				this.#requireWorkspace(connection, command.workspaceId);
				if (!this.#skills || !this.#workspacePath) throw Object.assign(new Error("Skill catalog is unavailable"), { protocolCode: "not_implemented" });
				return { type: "skill.list", workspaceId: command.workspaceId, skills: await this.#skills.list(command.workspaceId, this.#workspacePath(command.workspaceId)) };
			}
			case "skill.get": {
				this.#requireWorkspace(connection, command.workspaceId);
				if (!this.#skills || !this.#workspacePath) throw Object.assign(new Error("Skill catalog is unavailable"), { protocolCode: "not_implemented" });
				return { type: "skill.get", skill: await this.#skills.get(command.workspaceId, this.#workspacePath(command.workspaceId), command.skillId) };
			}
			case "mcp.list": {
				this.#requireWorkspace(connection, command.workspaceId);
				if (!this.#mcp || !this.#workspacePath) throw Object.assign(new Error("MCP catalog is unavailable"), { protocolCode: "not_implemented" });
				return { type: "mcp.list", workspaceId: command.workspaceId, servers: await this.#mcp.list(command.workspaceId, this.#workspacePath(command.workspaceId)) };
			}
			case "mcp.get": {
				this.#requireWorkspace(connection, command.workspaceId);
				if (!this.#mcp || !this.#workspacePath) throw Object.assign(new Error("MCP catalog is unavailable"), { protocolCode: "not_implemented" });
				return { type: "mcp.get", server: await this.#mcp.get(command.workspaceId, this.#workspacePath(command.workspaceId), command.serverId) };
			}
			case "model.list":
				return { type: "model.list", models: [...this.#models, ...(this.#customModels?.list() ?? [])] };
			case "model.custom.discover": {
				if (!this.#customModels) throw Object.assign(new Error("Custom model configuration is unavailable"), { protocolCode: "not_implemented" });
			...
			case "model.custom.service.refresh": {
				if (!this.#customModels) throw Object.assign(new Error("Custom model configuration is unavailable"), { protocolCode: "not_implemented" });
				return { type: "model.custom.discovered", ...await this.#customModels.refreshService(command.provider) };
			}
			...
			case "model.custom.set": {
				if (!this.#customModels) throw Object.assign(new Error("Custom model configuration is unavailable"), { protocolCode: "not_implemented" });
				if (this.#models.some((model) => model.model.provider === command.config.provider && model.model.id === command.config.id)) {
					throw new OrchestratorError("conflict", "A built-in model already uses that provider/model ID");
				}
			...
			case "model.custom.test": {
				if (!this.#customModels) throw Object.assign(new Error("Custom model configuration is unavailable"), { protocolCode: "not_implemented" });
				return { type: "model.custom.tested", model: command.model, latencyMs: await this.#customModels.test(command.model) };
			}
			case "session.list":
				this.#requireWorkspace(connection, command.workspaceId);
				return {
					type: "session.list",
					sessions: this.#store.listSnapshots(command.workspaceId, {
			...
			case "session.create": {
				...
				if (availableModels.length > 0) {
					const model = availableModels.find(
						(candidate) => candidate.model.provider === command.model.provider && candidate.model.id === command.model.id,
					);
					if (!model?.authenticated) throw new OrchestratorError("conflict", "Model is unavailable or not authenticated");
				}
				const result = await this.#orchestrator.createSession({
					...
					thinkingLevel: command.thinkingLevel,
					 ...
					 approvalPolicy: command.approvalPolicy,
						...
						...(command.budgetWarningThreshold === undefined ? {} : { budgetWarningThreshold: command.budgetWarningThreshold }),
				});
			...
			case "session.snapshot.get":
				return { type: "session.snapshot", snapshot: this.#requireSession(connection, command.sessionId) };
			case "session.rename":
				this.#requireSession(connection, command.sessionId);
			...
			case "session.model.set": {
				...
				if ([...this.#models, ...(this.#customModels?.list() ?? [])].length > 0 && !model?.authenticated) {
			...
			case "session.budget.set":
				this.#requireSession(connection, command.sessionId);
				return this.#orchestrator.setSessionBudget({ principalId: connection.principal.id, idempotencyKey, sessionId: command.sessionId, ...(command.costBudgetUsd === undefined ? {} : { costBudgetUsd: command.costBudgetUsd }), ...(command.tokenBudget === undefined ? {} : { tokenBudget: command.tokenBudget }), ...(command.budgetWarningThreshold === undefined ? {} : { budgetWarningThreshold: command.budgetWarningThreshold }) });
			case "session.run.list": {
				...
				return {
					...
					sessionId: command.sessionId,
					runs: this.#store.listOperations(command.sessionId, command.limit ?? 20).map((operation) => ({
						id: operation.id,
						...
						...(sessionSnapshot === undefined ? {} : { model: sessionSnapshot.usageByTurn?.find((turn) => turn.turnId === operation.id)?.model ?? sessionSnapshot.model }),
						...(operation.failureKind === undefined ? {} : { failureKind: operation.failureKind }),
						...(operation.retryHistory === undefined ? {} : { retryHistory: operation.retryHistory.map((retry) => ({ attempt: retry.attempt, maxAttempts: retry.maxAttempts, delayMs: retry.delayMs, error: retry.error, timestamp: retry.timestamp ?? operation.updatedAt })) }),
					})),
				};
			}
			...
			case "turn.follow_up":
				...
				if (command.content.some((part) => part.type === "artifact")) {
					if (!this.#artifacts) throw Object.assign(new Error("Artifact service is unavailable"), { protocolCode: "not_implemented" });
					for (const part of command.content) {
						if (part.type === "artifact") this.#artifacts.assertReference(part.artifact, session.session.workspaceId);
					}
				}
				if (command.skills && command.skills.length > 0) {
					if (!this.#skills || !this.#workspacePath) throw Object.assign(new Error("Skill catalog is unavailable"), { protocolCode: "not_implemented" });
					const available = new Set((await this.#skills.list(session.session.workspaceId, this.#workspacePath(session.session.workspaceId))).map((skill) => skill.id));
					const missing = command.skills.find((skillId) => !available.has(skillId));
					if (missing) throw Object.assign(new Error(`Skill ${missing} does not exist`), { protocolCode: "not_found" });
				}
			...
			case "approval.respond":
				this.#requireSession(connection, command.sessionId);
				if (!this.#approvals) {
					throw Object.assign(new Error("Approval handling is unavailable"), { protocolCode: "not_implemented" });
				}
			...
			default:
				throw Object.assign(new Error(`Command ${(command as { type: string }).type} is not implemented`), {
	...
	#publicEvent(stored: StoredSessionEvent): DurableEvent | undefined {
		...
		switch (event.type) {
			case "session.item.upserted":
				return { type: "session.item.upserted", sessionId: event.sessionId, revision: event.revision, item: event.item };
			case "session.phase.changed":
				return { type: "session.phase.changed", sessionId: event.sessionId, revision: event.revision, phase: event.phase };
			case "approval.requested":
				return { type: "approval.requested", sessionId: event.sessionId, revision: event.revision, approval: event.approval };
			case "approval.settled":
				return { type: "approval.settled", sessionId: event.sessionId, revision: event.revision, approval: event.approval };
			default: {
	...
	#sendStoredEvent(connection: ConnectionState, stored: StoredSessionEvent, requireAttachment: boolean): void {
		if (!connection.hello) return;
		const snapshot = this.#store.loadSnapshot(stored.event.sessionId);
		if (!snapshot) return;
		if (!connection.principal.workspaces.some((workspace) => workspace.id === snapshot.session.workspaceId)) return;
	...
	#send(connection: ConnectionState, message: import("@wuming/protocol").ServerMessage): void {
