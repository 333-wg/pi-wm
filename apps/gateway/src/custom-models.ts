...
import type { CustomModelApi, CustomModelCandidate, CustomModelConfig, CustomModelConnection, CustomModelService, CustomModelSettings, ModelMetadata, ModelRef } from "@wuming/protocol";
...
type StoredService = Omit<CustomModelService, "authenticated" | "modelCount"> & { apiKey: string };
...
function validateStoredService(service: StoredService): void {
	...
	if (!["openai-completions", "openai-responses", "anthropic-messages"].includes(service.api)) throw new Error("Stored model API type is invalid");
...
function providerId(baseUrl: string): string {
	const url = validateBaseUrl(baseUrl);
	const host = url.hostname.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "endpoint";
...
function parseModels(payload: unknown): CustomModelCandidate[] {
	...
	for (const value of payload.data) {
		...
		const candidateName = "display_name" in value && typeof value.display_name === "string"
			? value.display_name
	...
	return models.sort((left, right) => left.name.localeCompare(right.name, undefined, { sensitivity: "base" }));
}

function metadata(config: StoredConfig): ModelMetadata {
	return {
...
function providerRegistration(config: StoredConfig): PiProviderRegistration {
	return {
...
export class CustomModelRegistry {
	...
	async load(): Promise<void> {
		...
		const catalog = JSON.parse(Buffer.concat([decipher.update(Buffer.from(stored.data, "base64url")), decipher.final()]).toString("utf8")) as unknown;
		this.#configs.clear();
		this.#services.clear();
		if (Array.isArray(catalog)) {
			for (const value of catalog) {
				...
				const service: StoredService = { provider: config.provider, baseUrl: config.baseUrl, api: config.api, apiKey: config.apiKey };
				const existing = this.#services.get(service.provider);
				if (existing && (existing.baseUrl !== service.baseUrl || existing.api !== service.api || existing.apiKey !== service.apiKey)) {
		...
		if (!catalog || typeof catalog !== "object" || !("version" in catalog) || catalog.version !== 2 || !("services" in catalog) || !Array.isArray(catalog.services) || !("models" in catalog) || !Array.isArray(catalog.models)) {
		...
		for (const value of catalog.models) {
			...
			const config: StoredConfig = { ...model, api: service.api, baseUrl: service.baseUrl, apiKey: service.apiKey };
			validateStoredConfig(config);
			this.#configs.set(keyFor(config), config);
		}
	}
	...
	list(): ModelMetadata[] {
		return [...this.#configs.values()].map(metadata);
	}
	...
	registrations(): PiProviderRegistration[] {
		...
		for (const config of this.#configs.values()) providers.set(config.provider, [...(providers.get(config.provider) ?? []), config]);
		return [...providers].map(([provider, configs]) => {
			...
			return {
				...
				config: {
					...
					models: configs.map((config) => providerRegistration(config).config.models![0]!),
				},
			};
		});
	}
	...
	async set(config: CustomModelConfig): Promise<ModelMetadata> {
		...
		if (!suppliedApiKey && !savedService) throw new Error("API Key is required when adding a model to an unsaved service");
		if (!suppliedApiKey && savedService && (savedService.baseUrl !== config.baseUrl || savedService.api !== config.api)) {
		...
		const service: StoredService = suppliedApiKey
			? { provider: config.provider, baseUrl: config.baseUrl, api: config.api, apiKey: suppliedApiKey }
			: savedService!;
		validateStoredService(service);
		...
		if (savedService && suppliedApiKey && (savedService.baseUrl !== service.baseUrl || savedService.api !== service.api || savedService.apiKey !== service.apiKey)) {
			...
			for (const [key, candidate] of this.#configs) {
				if (candidate.provider === service.provider) this.#configs.set(key, { ...candidate, baseUrl: service.baseUrl, api: service.api, apiKey: service.apiKey });
			}
		}
		this.#services.set(service.provider, service);
	...
	async discover(connection: CustomModelConnection): Promise<CustomModelDiscovery> {
		const discovery = await this.#discover(connection);
		const service: StoredService = { provider: discovery.provider, baseUrl: discovery.baseUrl, api: discovery.api, apiKey: connection.apiKey.trim() };
		validateStoredService(service);
		this.#services.set(service.provider, service);
		for (const [key, config] of this.#configs) {
			if (config.provider === service.provider) this.#configs.set(key, { ...config, baseUrl: service.baseUrl, api: service.api, apiKey: service.apiKey });
		}
		await this.#persist();
		return discovery;
	}
	...
	async refreshService(provider: string): Promise<CustomModelDiscovery> {
		...
		const discovery = await this.#discover({ baseUrl: service.baseUrl, apiKey: service.apiKey });
		const updated = { ...service, baseUrl: discovery.baseUrl, api: discovery.api };
		this.#services.set(provider, updated);
		for (const [key, config] of this.#configs) {
			if (config.provider === provider) this.#configs.set(key, { ...config, baseUrl: updated.baseUrl, api: updated.api });
		}
		await this.#persist();
	...
	async removeService(provider: string): Promise<void> {
		...
		if ([...this.#configs.values()].some((config) => config.provider === provider)) {
			throw Object.assign(new Error("Remove the models under this service before deleting the service"), { protocolCode: "conflict" });
		}
	...
	async #discover(connection: CustomModelConnection): Promise<CustomModelDiscovery> {
		...
		const apiOrder: Array<"openai" | "anthropic"> = prefersAnthropic ? ["anthropic", "openai"] : ["openai", "anthropic"];
		const errors: string[] = [];
		try {
			for (const family of apiOrder) {
				const headers: Record<string, string> = family === "openai"
				...
				for (const endpoint of endpoints) {
					try {
						...
						const models = parseModels(await readBoundedJson(response));
						const api: CustomModelApi = family === "anthropic"
							? "anthropic-messages"
							: url.hostname.toLowerCase() === "api.openai.com"
								? "openai-responses"
								: "openai-completions";
						const discoveredBaseUrl = baseUrlFromModelsEndpoint(endpoint);
						return { provider: providerId(discoveredBaseUrl), baseUrl: discoveredBaseUrl, api, models, latencyMs: Math.max(0, Date.now() - started) };
					} catch (error) {
	...
	async test(model: ModelRef): Promise<number> {
		...
		try {
			const endpoint = resourceUrl(
				config.baseUrl,
				config.api === "openai-completions" ? "chat/completions" : config.api === "openai-responses" ? "responses" : "messages",
				config.api === "anthropic-messages",
			);
			const headers: Record<string, string> = { "content-type": "application/json" };
			if (config.api === "openai-completions" || config.api === "openai-responses") headers.authorization = `Bearer ${config.apiKey}`;
			else {
			...
			const response = await fetch(endpoint, {
				...
				body: JSON.stringify(config.api === "openai-completions"
					? { model: config.id, messages: [{ role: "user", content: "ping" }], max_tokens: 1, stream: false }
	...
	async #persist(): Promise<void> {
		...
		const catalog: StoredCatalog = { version: 2, services: [...this.#services.values()], models: [...this.#configs.values()].map(storedModel) };
