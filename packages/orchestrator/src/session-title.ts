...
function truncate(value: string, maxChars: number): string {
	...
	let shortened = characters.slice(0, maxChars - 1).join("").trimEnd();
...
export function suggestSessionTitle(content: readonly UserContentPart[]): string | undefined {
	...
	const attachments = compact(content
...
export function suggestSessionTitleFromTranscript(transcript: readonly TranscriptItem[]): string | undefined {
	for (const item of transcript) {
		...
		const title = suggestSessionTitle(item.content.filter(
			(part): part is UserContentPart => part.type === "text" || part.type === "artifact",
		));
